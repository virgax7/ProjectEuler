...
pandis = itertools.permutations(pandi)

primes = [2,3,5,7,11,13,17]
indices = list(range(1,8))

print(sum(map(lambda tup : int(''.join(tup)), list(filter(lambda x : all(int(''.join(x[i:i+3])) % p == 0 for p,i in zip(primes,indices)), pandis)))))
